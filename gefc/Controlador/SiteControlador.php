...
class SiteControlador extends Controlador
{
    ...
    public function entradaLocais(): void
    {
        ...
        $dados = filter_input_array(INPUT_POST, FILTER_DEFAULT);
        if (isset($dados['registro_id'])) {
            (new EntradaModelo())->atualizarLocal($dados);
            echo 'Sucesso';
            exit;
        } elseif (isset($dados['adicionarEntrada'])) {
            (new EntradaModelo())->entradaLocal($dados);
            Helpers::redirecionar('entrada/local');
        } elseif (isset($dados['relatorio'])) {
            $registros = (new EntradaModelo())->pesquisaEntradaLocal($dados['relatorio']);
            $lotesArray = json_encode($registros);
            echo $lotesArray;
            exit;
        } 
        ...
        echo $this->template->renderizar('entradasUbs.html', ['titulo' => SITE_NOME . ' Entrada Local', 'produtos'=>$produtos,'local'=>$local]);
    }
    ...
    public function receitas(): void
    {
        ...
        if( isset($dados['cadReceita'])){
            (new ReceitaModelo())->cadReceita($dados);
            
        }
        elseif( isset($dados['cadPrescritor'])){
            (new ReceitaModelo())->cadPrescritor($dados);
            
        }
        elseif(isset($dados["registro_id"])) {
            
            (new ReceitaModelo())->editReceita($dados);
            Helpers::redirecionar('receitas');
        }
        elseif(isset($dados["observacao"])) {
           
            (new ReceitaModelo())->cadPaciente($dados);
         
        }
        elseif(isset($dados["pacientePesquisa"])) {
    ...
    public function receita(int $id): void
    {
        ...
        $receita = (new ReceitaModelo())->pesquisaReceita($id);
        $infos = (new ReceitaModelo())->dadosReceita($id);
        $dados = filter_input_array(INPUT_POST, FILTER_DEFAULT);
        if( isset($dados['addProduto'])){
            (new ReceitaModelo())->registarProdutoReceita($dados, $id);
            Helpers::redirecionar('receita/'.$id);
        }

        echo $this->template->renderizar('receita.html', ['titulo' => SITE_NOME . ' Receita '.$id,'produtos' => $produtos, 'id'=>$id, 'receita'=>$receita, 'infos'=>$infos]);
    ...
    public function venda(int $nome): void
    {
        ...
        $saida = (new VendaModelo())->pesquisaDadosSaida($nome);
        $local = intval($saida[0]['localId']);
        $dados = filter_input_array(INPUT_POST, FILTER_DEFAULT);
        if (isset($dados['adicionar_produto'])) {
            (new VendaModelo())->venda($dados, $nome, $local);
        ...
        echo $this->template->renderizar('venda.html', ['titulo' => SITE_NOME . ' Saída: ' . $nome , 'registros' => $registros, 'venda' => $nome, 'produtos' => $produtos, 'saida'=>$saida]);
    }
    ...
    public function venda_adicionar(): void
    {
        ...
        $dados = filter_input_array(INPUT_POST, FILTER_DEFAULT);
        if (isset($dados)) {
            (new VendaModelo())->dispensaRegistro($dados);
        }
    }
    public function registroVendas(): void
    {
        ...
        if(isset($_POST['relatorio'])){
            $dados = filter_input_array(INPUT_POST, FILTER_DEFAULT);
           $pesquisa = (new VendaModelo())->pesquisa($dados['dePesquisa'], $dados['atePesquisa'],  $dados['localPesquisa']);
        }
        echo $this->template->renderizar('registroVendas.html', ['titulo' => SITE_NOME . 'Saídas por Produto', 'locais' => $locais, 'pesquisas'=> $pesquisa,'dados'=>$dados]);
    }
    ...
    public function entradaLocal(): void
    {
        ...
        $entradas = (new LocalModelo())->recebidos();
     

        // Agrupa os registros pela venda (nome_venda) e data
      
        echo $this->template->renderizar('entradaLocal.html', ['titulo' => SITE_NOME . ' Entradas ','entradas' => $entradas]);
    }
    public function confirmacaoLocal(string $entrada): void
    {

        $entradas = (new LocalModelo())->confirmacao( $entrada);
        $dados = filter_input_array(INPUT_POST, FILTER_DEFAULT);
        if (isset($dados)) {
        ...
        echo $this->template->renderizar('confirmacao.html', ['titulo' => SITE_NOME . 'Confirmar Entrada', 'entradas' => $entradas, 'entrada' => $entrada]);
    }
