$(document).ready(function () {
    ...
    function produtosSelecionar(estoques) {
        ...
        validEstoques.forEach(function (estoque) {
            var optionValor = `${estoque.local_estoque_id};${estoque.lote_id};${estoque.nome};${estoque.estoque}`;
            var optionTexto = `COD:${estoque.lote_id}, ${estoque.lote} - ${estoque.nome} Qnt.: ${estoque.estoque} ${estoque.unidade_contagem} V:${estoque.vencimento} ${estoque.fornecedor}`;
    ...
    function atualizarCampoSelecionado(lote_id) {
        var $selecionadoCampo = $('#produtoSelecionar');
        $selecionadoCampo.empty(); // Limpa o selectField
        ...
        var estoquesFiltrados = lote_id ? estoques.filter(function (estoque) {
            return estoque.lote_id == lote_id;
    ...
    $('#codbarrasHospital').on('input', function () {
        ...
        atualizarCampoSelecionado(lote_id);
    });
    ...
    $('#dispensaHospital').click(function (event) {
        ...
        newDiv.append('<input type="hidden" name="lote' + counter + '" value="' + splitValues[1] + '" readonly>');
